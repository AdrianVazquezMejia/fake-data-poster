...
def get_energy(meter_serial):
    ...
    data_base_connection = sqlite3.connect("meter_db.sqlite")
    data_base_cursor = data_base_connection.cursor()
    data_base_cursor.execute('SELECT * FROM meter_table WHERE meter_id = ?', (meter_serial,))
    energy = data_base_cursor.fetchone()[1]
    data_base_cursor.close()
    return energy 
...
if __name__ == '__main__':
    ...
    print("Energy is 007800: ",get_energy("007800"))
    update_date_base("007800", 11)
    for meter_serial in return_json["updates"]:
        data_test_main = 21000
